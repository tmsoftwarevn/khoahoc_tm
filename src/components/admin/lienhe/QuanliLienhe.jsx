...
const QuanliLienhe = () => {
  ...
  const fetchAllLienhe = async () => {
    const res = await fetch(`${process.env.URL_BACKEND}/api/v1/lienhe`);
    const result = await res.json();
    if (result && result.data) customLienhe(result.data);
  };

  const confirm = async (id) => {
    const data = await fetch(`${process.env.URL_BACKEND}/api/v1/lienhe/${id}`, {
      method: "DELETE",
    });
